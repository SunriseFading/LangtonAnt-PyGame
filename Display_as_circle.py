...
class App:
    def __init__(self, WIDTH=2560, HEIGHT=1600, CELL_SIZE=16):
        pg.init()
        ...
        ants1 = [Ant(self, [self.COLS // 3, self.ROWS // 2],
                     choice(colors1)) for _ in range(400)]
        ants2 = [Ant(self, [self.COLS - self.COLS // 3, self.ROWS // 2],
                     choice(colors2)) for _ in range(400)]
        self.ants = ants1 + ants2

    @staticmethod
    def get_color():
        def channel(): return randrange(30, 220)
